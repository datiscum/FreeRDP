...
#define XF_VAAPI_DECODE_FAILURE_FALLBACK_THRESHOLD 3U
...
typedef struct xf_vaapi_decode_failure_entry
{
	const xfGfxSurface* surface;
	UINT32 failures;
	struct xf_vaapi_decode_failure_entry* next;
} xfVaapiDecodeFailureEntry;

static xfVaapiDecodeFailureEntry* g_xf_vaapi_decode_failures = NULL;

static xfVaapiDecodeFailureEntry* xf_vaapi_decode_failure_find(const xfGfxSurface* surface)
{
	for (xfVaapiDecodeFailureEntry* cur = g_xf_vaapi_decode_failures; cur; cur = cur->next)
	{
		if (cur->surface == surface)
			return cur;
	}
...
static xfVaapiDecodeFailureEntry* xf_vaapi_decode_failure_get(const xfGfxSurface* surface)
{
	xfVaapiDecodeFailureEntry* entry = xf_vaapi_decode_failure_find(surface);
	if (entry)
		return entry;

	entry = (xfVaapiDecodeFailureEntry*)calloc(1, sizeof(xfVaapiDecodeFailureEntry));
	if (!entry)
		return NULL;

	entry->surface = surface;
	entry->next = g_xf_vaapi_decode_failures;
	g_xf_vaapi_decode_failures = entry;
	return entry;
}

static UINT32 xf_vaapi_decode_failure_increment(const xfGfxSurface* surface)
{
	xfVaapiDecodeFailureEntry* entry = xf_vaapi_decode_failure_get(surface);
	if (!entry)
		return XF_VAAPI_DECODE_FAILURE_FALLBACK_THRESHOLD;
...
static void xf_vaapi_decode_failure_reset(const xfGfxSurface* surface)
{
	xfVaapiDecodeFailureEntry* entry = xf_vaapi_decode_failure_find(surface);
	if (entry)
		entry->failures = 0;
...
static void xf_vaapi_decode_failure_destroy(const xfGfxSurface* surface)
{
	xfVaapiDecodeFailureEntry** cur = &g_xf_vaapi_decode_failures;

	while (*cur)
	{
		xfVaapiDecodeFailureEntry* entry = *cur;
		if (entry->surface == surface)
		{
			*cur = entry->next;
			free(entry);
...
static void xf_vaapi_decode_failure_destroy_all(void)
{
	while (g_xf_vaapi_decode_failures)
	{
		xfVaapiDecodeFailureEntry* entry = g_xf_vaapi_decode_failures;
		g_xf_vaapi_decode_failures = entry->next;
		free(entry);
	}
}
...
static void xf_vaapi_reset_h264_after_decode_failure(xfGfxSurface* surface)
{
	WINPR_ASSERT(surface);
	...
	{
		WLog_WARN(TAG,
		          "VAAPI direct: failed to reset H264 context after decode failures; recreating context");
...
fail:
	...
	surface->vaapiH264 = NULL;
	surface->vaapiDirectReady = FALSE;
	WLog_WARN(TAG, "VAAPI direct: ensure_h264 failed, not disabling permanently");
	return FALSE;
}
                                           ...
                                           const RDPGFX_SURFACE_COMMAND* cmd)
{
	...
	const BOOL ready = allowed ? xf_vaapi_ensure_h264(xfc, surface) : FALSE;
	...
	{
		WLog_WARN(TAG,
		          "VAAPI direct: H264 direct context not ready; using original path before consuming payload");
		status = XF_VAAPI_FALLBACK;
		goto fail;
	}
...
*/
	...
	if (rc < 0)
	{
		const UINT32 decodeFailures = xf_vaapi_decode_failure_increment(surface);
		if (decodeFailures >= XF_VAAPI_DECODE_FAILURE_FALLBACK_THRESHOLD)
		{
			WLog_WARN(TAG,
			          "VAAPI direct: AVC420 decode failed rc=%" PRId32
			          ", consecutive failures=%" PRIu32 "/%u; resetting H264 context and using original path",
			          rc, decodeFailures, XF_VAAPI_DECODE_FAILURE_FALLBACK_THRESHOLD);
			xf_vaapi_decode_failure_reset(surface);
			xf_vaapi_reset_h264_after_decode_failure(surface);
			status = XF_VAAPI_FALLBACK;
			goto fail;
		}
		...
		WLog_WARN(TAG,
		          "VAAPI direct: AVC420 decode failed rc=%" PRId32
		          ", consecutive failures=%" PRIu32 "/%u; dropping update without fallback",
		          rc, decodeFailures, XF_VAAPI_DECODE_FAILURE_FALLBACK_THRESHOLD);
                             ...
                             const RDPGFX_DELETE_SURFACE_PDU* deleteSurface)
{
	...
	if (surface)
	{
		...
		xf_vaapi_decode_failure_destroy(surface);
		region16_uninit(&surface->vaapiInvalidRegion);
#endif
#ifdef WITH_XSHM
...
void xf_graphics_pipeline_uninit(xfContext* xfc, RdpgfxClientContext* gfx)
{
	...
	xf_vaapi_decode_failure_destroy_all();
