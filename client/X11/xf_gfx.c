...
const BOOL full = xf_vaapi_avc420_full_surface(surface, meta);
const BOOL ready = (allowed && full) ? xf_vaapi_ensure_h264(xfc, surface) : FALSE;
...
if (!ready)
{
        WLog_WARN(TAG, "VAAPI direct: H264 direct context not ready, dropping AVC420 update without fallback");
